...
# Check if the tarfile exists in the local directory
if [ ! -f pics.tgz ]; then
  # Download the tarfile if it doesn't exist
  wget -q https://zonzorp.net/pics.tgz
fi

# Extract the tarfile if the download was successful
if [ -f pics.tgz ]; then
  tar -xzf pics.tgz -C ~/public_html/pics
  extraction_status=$?

  # Delete the local copy of the tarfile if extraction was successful
